...
from aksara.utils import exec_cmd
from aksara.app import App
from aksara.tests.test_base import FRAPPE_BRANCH, TestBenchBase
...
class TestBenchInit(TestBenchBase):
    ...
    def test_get_app(self):
        ...
        exec_cmd(f"bench get-app {TEST_FRAPPE_APP} --skip-assets", cwd=aksara_path)
        self.assertTrue(
            os.path.exists(os.path.join(aksara_path, "apps", TEST_FRAPPE_APP))
        )
        app_installed_in_env = TEST_FRAPPE_APP in subprocess.check_output(
            ["aksara", "pip", "freeze"], cwd=aksara_path
        ).decode("utf8")
    ...
    def test_get_app_resolve_deps(self):
        ...
        aksara_path = os.path.join(self.benches_path, "test-bench")
        exec_cmd(
            f"bench get-app {FRAPPE_APP} --resolve-deps --skip-assets",
            cwd=aksara_path,
        )
        self.assertTrue(os.path.exists(os.path.join(aksara_path, "apps", FRAPPE_APP)))

        states_path = os.path.join(aksara_path, "sites", "apps.json")
    ...
    def test_install_app(self):
        ...
        if installed_app:
            app_installed_on_site = subprocess.check_output(
                ["aksara", "--site", site_name, "list-apps"], cwd=aksara_path
            ).decode("utf8")
    ...
    def test_remove_app(self):
        ...
        exec_cmd(f"bench remove-app {TEST_FRAPPE_APP}", cwd=aksara_path)

        with open(os.path.join(aksara_path, "sites", "apps.txt")) as f:
    ...
    def test_switch_to_branch(self):
        self.init_bench("test-bench")
        aksara_path = os.path.join(self.benches_path, "test-bench")
        app_path = os.path.join(aksara_path, "apps", "logica")
