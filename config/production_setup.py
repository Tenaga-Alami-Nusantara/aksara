...
def setup_production(user, aksara_path=".", yes=False):
    print("Setting Up prerequisites...")
    setup_production_prerequisites()

    conf = Aksara(aksara_path).conf

    if conf.get("restart_supervisor_on_update") and conf.get(
    ...
    if conf.get("restart_systemd_on_update"):
        ...
        generate_systemd_config(aksara_path=aksara_path, user=user, yes=yes)
    else:
        print("Setting Up supervisor...")
        check_supervisord_config(user=user)
        generate_supervisor_config(aksara_path=aksara_path, user=user, yes=yes)

    print("Setting Up NGINX...")
    make_nginx_conf(aksara_path=aksara_path, yes=yes)
    ...
    if conf.get("restart_supervisor_on_update"):
        ...
        if not os.path.islink(supervisor_conf):
            os.symlink(
                os.path.abspath(os.path.join(aksara_path, "config", "supervisor.conf")),
                supervisor_conf,
            )

    if not os.path.islink(nginx_conf):
        os.symlink(
            os.path.abspath(os.path.join(aksara_path, "config", "nginx.conf")),
            nginx_conf,
        )
...
def service(service_name, service_option):
    if (
        os.path.basename(which("systemctl") or "") == "systemctl"
        and is_running_systemd()
    ):
