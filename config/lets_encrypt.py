...
from aksara.config.production_setup import service
from aksara.config.site_config import get_domains, remove_domain, update_site_config
from aksara.aksara import Aksara
from aksara.utils import exec_cmd, which
from aksara.utils.aksara import update_common_site_config
from aksara.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, aksara_path, interactive):
    site_path = os.path.join(aksara_path, "sites", site, "site_config.json")
    ...
    if not Aksara(aksara_path).conf.get("dns_multitenant"):
        print("You cannot setup SSL without DNS Multitenancy")
        return

    create_config(site, custom_domain)
    run_certbot_and_setup_ssl(site, custom_domain, aksara_path, interactive)
    setup_crontab()
...
def run_certbot_and_setup_ssl(site, custom_domain, aksara_path, interactive=True):
    service("nginx", "stop")
    ...
    if custom_domain:
        remove_domain(site, custom_domain, aksara_path)
        domains = get_domains(site, aksara_path)
        ssl_config["domain"] = custom_domain
        domains.append(ssl_config)
        update_site_config(site, {"domains": domains}, aksara_path=aksara_path)
    else:
        update_site_config(site, ssl_config, aksara_path=aksara_path)

    make_nginx_conf(aksara_path)
    service("nginx", "start")
...
def setup_crontab():
    ...
    for job in system_crontab.find_comment(
        comment=job_comment
...
def setup_wildcard_ssl(domain, email, aksara_path, exclude_base_domain):
