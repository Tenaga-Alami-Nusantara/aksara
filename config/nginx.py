...
def make_nginx_conf(aksara_path, yes=False, logging=None, log_format=None):
    conf_path = os.path.join(aksara_path, "config", "nginx.conf")
    ...
    template = aksara.config.env().get_template("nginx.conf")
...
def make_bench_manager_nginx_conf(aksara_path, yes=False, port=23624, domain=None):
    from aksara.config.site_config import get_site_config

    template = aksara.config.env().get_template("bench_manager_nginx.conf")
    aksara_path = os.path.abspath(aksara_path)
    sites_path = os.path.join(aksara_path, "sites")
...
def prepare_sites(config, aksara_path):
    sites = {
        "that_use_port": [],
        "that_use_dns": [],
        "that_use_ssl": [],
    ...
    sites_configs = get_sites_with_config(aksara_path=aksara_path)
    ...
    for site in sites_configs:
        if dns_multitenant:
            ...
            if (
                ...
                and not site["name"] in ports_in_use[site["port"]]
            ):
...
def get_sites_with_config(aksara_path):
    from aksara.aksara import Aksara
    from aksara.config.site_config import get_site_config
    ...
    for site in sites:
        try:
            site_config = get_site_config(site, aksara_path=aksara_path)
    ...
    use_wildcard_certificate(aksara_path, ret)

    return ret


def use_wildcard_certificate(aksara_path, ret):
    """
...
def get_error_pages():
    bench_app_path = os.path.abspath(aksara.__path__[0])
