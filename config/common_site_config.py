...
def setup_config(aksara_path):
    make_pid_folder(aksara_path)
...
def get_config(aksara_path):
    return get_common_site_config(aksara_path)


def get_common_site_config(aksara_path):
    config_path = get_config_path(aksara_path)
...
def put_config(config, aksara_path="."):
    config_path = get_config_path(aksara_path)
    with open(config_path, "w") as f:
        return json.dump(config, f, indent=1, sort_keys=True)


def update_config(new_config, aksara_path="."):
    config = get_config(aksara_path=aksara_path)
    config.update(new_config)
    put_config(config, aksara_path=aksara_path)


def get_config_path(aksara_path):
    return os.path.join(aksara_path, "sites", "common_site_config.json")
...
def make_ports(aksara_path):
    from urllib.parse import urlparse

    benches_path = os.path.dirname(os.path.abspath(aksara_path))
    ...
    for folder in os.listdir(benches_path):
        aksara_path = os.path.join(benches_path, folder)
...
def make_pid_folder(aksara_path):
    pids_path = os.path.join(aksara_path, "config", "pids")
