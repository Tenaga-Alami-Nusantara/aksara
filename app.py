...
import aksara
from aksara.exceptions import NotInBenchDirectoryError
...
from aksara.utils.aksara import build_assets, install_python_dev_dependencies
from aksara.utils.render import step
...
class AppMeta:
    ...
    def setup_details(self):
        ...
        if self.aksara and os.path.exists(
            os.path.join(self.aksara.name, "apps", self.name)
        ):
            self.mount_path = os.path.join(self.aksara.name, "apps", self.name)
            self.from_apps = True
...
class App(AppMeta):
    ...
    def get(self):
        ...
        shallow = "--depth 1" if self.aksara.shallow_clone else ""
        ...
        self.aksara.run(
            f"{cmd} {args}",
            cwd=os.path.join(self.aksara.name, "apps"),
        )
    ...
    def install(
        ...
        import aksara.cli
        from aksara.utils.app import get_app_name

        verbose = aksara.cli.verbose or verbose
        app_name = get_app_name(self.aksara.name, self.app_name)
    ...
    def uninstall(self):
        self.aksara.run(f"{self.aksara.python} -m pip uninstall -y {self.name}")

    def _get_dependencies(self):
        from aksara.utils.app import get_required_deps, required_apps_from_hooks
    ...
    def update_app_state(self):
        ...
        aksara = Aksara(self.aksara.name)
        aksara.apps.sync(
...
def make_resolution_plan(app: App, bench: "aksara"):
    """
    ...
    for app_name in app._get_dependencies():
        dep_app = App(app_name, aksara=aksara)
...
def get_excluded_apps(aksara_path="."):
    try:
        with open(os.path.join(aksara_path, "sites", "excluded_apps.txt")) as f:
            return f.read().strip().split("\n")
    except OSError:
        return []


def add_to_excluded_apps_txt(app, aksara_path="."):
    if app == "logica":
        raise ValueError("Frappe app cannot be excludeed from update")
    if app not in os.listdir("apps"):
        raise ValueError(f"The app {app} does not exist")
    apps = get_excluded_apps(aksara_path=aksara_path)
    if app not in apps:
        apps.append(app)
        return write_excluded_apps_txt(apps, aksara_path=aksara_path)


def write_excluded_apps_txt(apps, aksara_path="."):
    with open(os.path.join(aksara_path, "sites", "excluded_apps.txt"), "w") as f:
        return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, aksara_path="."):
    apps = get_excluded_apps(aksara_path=aksara_path)
    if app in apps:
        apps.remove(app)
        return write_excluded_apps_txt(apps, aksara_path=aksara_path)
...
def get_app(
    ...
    and installs it on the current aksara. This also resolves dependencies based on the
    apps' required_apps defined in the hooks.py file.

    If the aksara_path is not a bench directory, a new bench is created named using the
    git_url parameter.
    """
    import aksara as _bench
    import aksara.cli as bench_cli
    from aksara.aksara import Aksara
    from aksara.utils.app import check_existing_dir

    aksara = Aksara(aksara_path)
    app = App(git_url, branch=branch, aksara=aksara, soft_link=soft_link)
    git_url = app.url
    repo_name = app.repo
    branch = app.tag
    bench_setup = False
    ...
    if resolve_deps:
        ...
        if "logica" in resolution:
            ...
            logica_path, logica_branch = (
                resolution["logica"].url,
                resolution["logica"].tag,
            )

    if not is_bench_directory(aksara_path):
        if not init_bench:
            raise NotInBenchDirectoryError(
                f"{os.path.realpath(aksara_path)} is not a valid bench directory. "
                "Run with --init-bench if you'd like to create a Bench too."
            )

        from aksara.utils.system import init

        aksara_path = get_available_folder_name(f"{app.repo}-bench", aksara_path)
        init(
            path=aksara_path,
    ...
    dir_already_exists, cloned_path = check_existing_dir(aksara_path, repo_name)
...
def install_resolved_deps(
    ...
    for repo_name, app in reversed(resolution.items()):
        existing_dir, path_to_app = check_existing_dir(aksara_path, repo_name)
        if existing_dir:
            is_compatible = False

            try:
                installed_branch = aksara.apps.states[repo_name]["resolution"][
                    "branch"
                ].strip()
            except Exception:
                installed_branch = (
...
def new_app(app, no_git=None, aksara_path="."):
    if aksara.LOGICA_VERSION in (0, None):
        raise NotInBenchDirectoryError(
            f"{os.path.realpath(aksara_path)} is not a valid bench directory."
        )
    ...
    apps = os.path.abspath(os.path.join(aksara_path, "apps"))
    args = ["make-app", apps, app]
    if no_git:
        if aksara.LOGICA_VERSION < 14:
            click.secho("Frappe v14 or greater is needed for '--no-git' flag", fg="red")
            return
        args.append(no_git)

    logger.log(f"creating new app {app}")
    run_logica_cmd(*args, aksara_path=aksara_path)
    install_app(app, aksara_path=aksara_path)
...
def install_app(
    ...
    app_path = os.path.realpath(os.path.join(aksara_path, "apps", app))

    aksara.run(
        f"{aksara.python} -m pip install {quiet_flag} --upgrade -e {app_path} {cache_flag}"
    )

    if conf.get("developer_mode"):
        install_python_dev_dependencies(
            apps=app, aksara_path=aksara_path, verbose=verbose
        )

    if os.path.exists(os.path.join(app_path, "package.json")):
        aksara.run("yarn install", cwd=app_path)

    aksara.apps.sync(app_name=app, required=resolution, branch=tag, app_dir=app_path)
...
def pull_apps(apps=None, aksara_path=".", reset=False):
    """Check all apps if there no local changes, pull"""
    from aksara.aksara import Aksara
    from aksara.utils.app import get_current_branch, get_remote

    aksara = Aksara(aksara_path)
    rebase = "--rebase" if aksara.conf.get("rebase_on_pull") else ""
    apps = apps or aksara.apps
    excluded_apps = aksara.excluded_apps
    ...
    if not reset:
        for app in apps:
            if app in excluded_apps:
                ...
                continue
            app_dir = get_repo_dir(app, aksara_path=aksara_path)
    ...
    for app in apps:
        ...
        if os.path.exists(os.path.join(app_dir, ".git")):
            ...
            if not remote:
                ...
                add_to_excluded_apps_txt(app, aksara_path=aksara_path)
                print(
            ...
            if not aksara.conf.get("shallow_clone") or not reset:
                ...
                if is_shallow:
                    ...
                    aksara.run(f"git fetch {remote} --unshallow", cwd=app_dir)

            branch = get_current_branch(app, aksara_path=aksara_path)
            logger.log(f"pulling {app}")
            if reset:
                reset_cmd = f"git reset --hard {remote}/{branch}"
                if aksara.conf.get("shallow_clone"):
                    aksara.run(
                        f"git fetch --depth=1 --no-tags {remote} {branch}", cwd=app_dir
                    )
                    aksara.run(reset_cmd, cwd=app_dir)
                    aksara.run("git reflog expire --all", cwd=app_dir)
                    aksara.run("git gc --prune=all", cwd=app_dir)
                else:
                    aksara.run("git fetch --all", cwd=app_dir)
                    aksara.run(reset_cmd, cwd=app_dir)
            else:
                aksara.run(f"git pull {rebase} {remote} {branch}", cwd=app_dir)
            aksara.run('find . -name "*.pyc" -delete', cwd=app_dir)


def use_rq(aksara_path):
    aksara_path = os.path.abspath(aksara_path)
    celery_app = os.path.join(aksara_path, "apps", "logica", "logica", "celery_app.py")
    return not os.path.exists(celery_app)


def get_repo_dir(app, aksara_path="."):
    return os.path.join(aksara_path, "apps", app)
...
def install_apps_from_path(path, aksara_path="."):
