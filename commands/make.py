...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Ignore if Bench instance exists.",
)
@click.option(
...
@click.option(
    "--apps_path",
    default=None,
    help="path to json files with apps to install after init",
)
...
def init(
    ...
    try:
        ...
        from aksara.utils import get_traceback
...
def drop(path):
    ...
    from aksara.exceptions import aksaraNotFoundError, ValidationError

    aksara = Aksara(path)

    if not aksara.exists:
        raise BenchNotFoundError(f"Bench {aksara.name} does not exist")
...
def get_app(
    ...
    from aksara.app import get_app
...
def new_app(app_name, no_git=None):
    from aksara.app import new_app
...
@click.command(
    ...
    help=(
        "Completely remove app from aksara and re-build assets if not installed on any site"
    ),
)
...
def remove_app(app_name, no_backup=False, force=False):
    ...
    aksara = Aksara(".")
    aksara.uninstall(app_name, no_backup=no_backup, force=force)


@click.command("exclude-app", help="Exclude app from updating")
@click.argument("app_name")
def exclude_app_for_update(app_name):
    from aksara.app import add_to_excluded_apps_txt
...
def include_app_for_update(app_name):
    ...
    from aksara.app import remove_from_excluded_apps_txt
