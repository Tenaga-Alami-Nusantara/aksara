...
from aksara.utils.aksara import post_upgrade, patch_sites, build_assets


@click.command(
    "update",
    help="Performs an update operation on current aksara. Without any flags will backup, pull, setup requirements, build, run patches and restart aksara. Using specific flags will only do certain tasks instead of all",
)
...
@click.option(
    "--restart-supervisor",
    is_flag=True,
    help="Restart supervisor processes after update",
)
@click.option(
...
def update(
    ...
    from aksara.utils.aksara import update
...
def switch_to_branch(branch, apps, upgrade=False):
    from aksara.utils.app import switch_to_branch
