...
from aksara.utils import exec_cmd, run_playbook, which
from aksara.utils.cli import SugaredOption
...
def setup_sudoers(user):
    from aksara.utils.system import setup_sudoers
...
@click.option(
    "--yes",
    help="Yes to regeneration of nginx config file",
    default=False,
    is_flag=True,
)
def setup_nginx(yes=False, logging="combined", log_format=None):
    ...
    make_nginx_conf(aksara_path=".", yes=yes, logging=logging, log_format=log_format)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
    from aksara.config.production_setup import reload_nginx
...
@click.option(
    "--yes",
    help="Yes to regeneration of supervisor config",
    is_flag=True,
    default=False,
)
@click.option(
...
def setup_supervisor(user=None, yes=False, skip_redis=False, skip_supervisord=False):
    from aksara.utils import get_cmd_output
    from aksara.config.supervisor import (
    ...
    generate_supervisor_config(
        aksara_path=".", user=user, yes=yes, skip_redis=skip_redis
    )
...
def setup_fonts():
    from aksara.utils.system import setup_fonts
...
def setup_production(user, yes=False):
    from aksara.config.production_setup import setup_production
...
def setup_env(python="python3"):
    ...
    return Aksara(".").setup.env(python=python)
...
def setup_letsencrypt(site, custom_domain, non_interactive):
    from aksara.config.lets_encrypt import setup_letsencrypt

    setup_letsencrypt(
        site, custom_domain, aksara_path=".", interactive=not non_interactive
    )
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
    from aksara.config.lets_encrypt import setup_wildcard_ssl

    setup_wildcard_ssl(
        domain, email, aksara_path=".", exclude_base_domain=exclude_base_domain
    )
...
def setup_requirements(node=False, python=False, dev=False, apps=None):
    ...
    if not (node or python or dev):
        aksara.setup.requirements(apps=apps)

    elif not node and not dev:
        aksara.setup.python(apps=apps)

    elif not python and not dev:
        aksara.setup.node(apps=apps)

    else:
        from aksara.utils.aksara import install_python_dev_dependencies
...
def setup_manager(yes=False, port=23624, domain=None):
    from aksara.aksara import Aksara
    from aksara.config.nginx import make_bench_manager_nginx_conf
    ...
    if aksara.conf.get("restart_supervisor_on_update") or aksara.conf.get(
        ...
        if domain not in aksara.sites:
            raise Exception("No such site")

        make_bench_manager_nginx_conf(aksara_path, yes=yes, port=port, domain=domain)
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
    ...
    from aksara.config.site_config import add_domain

    add_domain(site, domain, ssl_certificate, ssl_certificate_key, aksara_path=".")
...
def remove_domain(domain, site=None):
    ...
    from aksara.config.site_config import remove_domain

    remove_domain(site, domain, aksara_path=".")
...
def sync_domains(domain=None, site=None):
    ...
    from aksara.config.site_config import sync_domains

    changed = sync_domains(site, domains, aksara_path=".")
