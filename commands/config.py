...
from aksara.config.common_site_config import update_config, put_config
...
def set_common_config(configs):
    ...
    update_config(common_site_config, aksara_path=".")


@click.command(
    "remove-common-config",
    help="Remove specific keys from current bench's common config",
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
    from aksara.aksara import Aksara

    common_site_config = Aksara(".").conf
    for key in keys:
