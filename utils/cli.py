...
def use_experimental_feature(ctx, param, value):
    ...
    if value == "dynamic-feed":
        ...
        aksara.cli.dynamic_feed = True
        aksara.cli.verbose = True
    else:
        from aksara.exceptions import FeatureDoesNotExistError

        raise FeatureDoesNotExistError(f"Feature {value} does not exist")

    from aksara.cli import is_envvar_warn_set
