...
from aksara import PROJECT_NAME, VERSION
...
def is_valid_logica_branch(logica_path: str, logica_branch: str):
    ...
    if logica_branch:
        try:
            ...
            raise InvalidRemoteException(f"Invalid frappe path: {logica_path}") from e
...
def log(message, level=0, no_log=False, stderr=False):
    ...
    if aksara.cli.from_command_line and aksara.cli.dynamic_feed:
        aksara.LOG_BUFFER.append({"prefix": prefix, "message": message, "color": color})
...
def setup_logging(aksara_path=".") -> logging.Logger:
    LOG_LEVEL = 15
    ...
    if os.path.exists(os.path.join(aksara_path, "logs")):
        log_file = os.path.join(aksara_path, "logs", "aksara.log")
...
def run_logica_cmd(*args, **kwargs):
    from aksara.cli import from_command_line
    from aksara.utils.aksara import get_env_cmd

    aksara_path = kwargs.get("aksara_path", ".")
    f = get_env_cmd("python", aksara_path=aksara_path)
    sites_dir = os.path.join(aksara_path, "sites")
    ...
    p = subprocess.Popen(
        (f, "-m", "logica.utils.bench_helper", "logica") + args,
        cwd=sites_dir,
        stdout=stdout,
        stderr=stderr,
    )
...
def set_git_remote_url(git_url, aksara_path="."):
    ...
    if app not in Aksara(aksara_path).apps:
...
def run_playbook(playbook_name, extra_vars=None, tag=None):
    ...
    subprocess.check_call(args, cwd=os.path.join(aksara.__path__[0], "playbooks"))
...
def get_env_logica_commands(aksara_path=".") -> List:
    ...
    try:
        return json.loads(
            get_cmd_output(
                f"{python} -m logica.utils.bench_helper get-logica-commands",
                cwd=sites_path,
            )
        )
...
def get_cmd_from_sysargv():
    ...
    logica_context = _dict(
        params={"--site"}, flags={"--verbose", "--profile", "--force"}
    )
    cmd_from_ctx = None
    sys_argv = sys.argv[1:]
    skip_next = False
    ...
    for arg in sys_argv:
        ...
        if sys_argv.index(arg) == 0 and arg in Aksara(".").apps:
            continue
