...
from aksara.app import get_repo_dir


def is_version_upgrade(app="logica", aksara_path=".", branch=None):
    upstream_version = get_upstream_version(
        app=app, branch=branch, aksara_path=aksara_path
    )
    ...
    local_version = get_major_version(get_current_version(app, aksara_path=aksara_path))
...
def switch_branch(
    branch, apps=None, aksara_path=".", upgrade=False, check_upgrade=True
):
    import git
    from aksara.aksara import Aksara
    from aksara.utils import log, exec_cmd
    ...
    for app in apps:
        ...
        exec_cmd(
            f"git fetch --all{' --unshallow' if unshallow_flag else ''} --quiet",
            cwd=app_dir,
        )

        if check_upgrade:
            version_upgrade = is_version_upgrade(
                app=app, aksara_path=aksara_path, branch=branch
            )
    ...
    if version_upgrade[0] and upgrade:
        Aksara(aksara_path).setup.requirements()
        backup_all_sites()
        patch_sites()
        build_assets()
...
def switch_to_branch(branch=None, apps=None, aksara_path=".", upgrade=False):
    switch_branch(branch, apps=apps, aksara_path=aksara_path, upgrade=upgrade)


def switch_to_develop(apps=None, aksara_path=".", upgrade=True):
    switch_branch("develop", apps=apps, aksara_path=aksara_path, upgrade=upgrade)
...
def get_develop_version(app, aksara_path="."):
    repo_dir = get_repo_dir(app, aksara_path=aksara_path)
...
def get_upstream_version(app, branch=None, aksara_path="."):
    repo_dir = get_repo_dir(app, aksara_path=aksara_path)
    if not branch:
    ...
    try:
        ...
        raise InvalidRemoteException(
            f"Failed to fetch from remote named upstream for {app}"
        )
...
def get_current_logica_version(aksara_path="."):
    try:
        return get_major_version(get_current_version("logica", aksara_path=aksara_path))
    except OSError:
        return 0


def get_current_branch(app, aksara_path="."):
...
def get_remote(app, aksara_path="."):
    repo_dir = get_repo_dir(app, aksara_path=aksara_path)
    contents = subprocess.check_output(
...
def get_app_name(aksara_path: str, folder_name: str) -> str:
    ...
    """
    app_name = None
    apps_path = os.path.join(os.path.abspath(aksara_path), "apps")
    ...
    if not app_name:
        ...
        with open(setup_py_path, "rb") as f:
            app_name = re.search(
                r'name\s*=\s*[\'"](.*)[\'"]', f.read().decode("utf-8")
            )[1]

    if app_name and folder_name != app_name:
        os.rename(
            os.path.join(apps_path, folder_name), os.path.join(apps_path, app_name)
        )
        return app_name
...
def check_existing_dir(aksara_path, repo_name):
    cloned_path = os.path.join(aksara_path, "apps", repo_name)
    dir_already_exists = os.path.isdir(cloned_path)
    return dir_already_exists, cloned_path


def get_current_version(app, aksara_path="."):
